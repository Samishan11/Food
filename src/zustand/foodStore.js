...
const useFoodstore = create(set => ({
    ...
    myrating:[],
    getfood: async () => {
        const response = await axios.get('https://mern-foodapp.herokuapp.com/getallfood')
        set({ food: response.data })
    },
    getsingleFood: async (food , user) => {
        const response = await axios.get(`https://mern-foodapp.herokuapp.com/fooddetail/${food}`)
